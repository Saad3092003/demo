...
import { AllCategories_mpm, AllCategories_sm, MarketplaceAllListing, allProjects } from "../../handleSlice";
import Category from "../../components/Category";

function Category_mpm({ loadScreen }) {
    // const cats = [
    //     {
    //         img: electricalimg,
    //         catname: 'Electrical',
    ...
    // console.log(Category_smDetails);
    const category = useSelector((state) => state.userDetails.AllCategories_mpm);
    console.log(category);
    console.log(marketplaceAllListings);
    // console.log(Category_smDetails);

    return (
        <>
            <div className='account-bg p-5'>
                <h1 className='text-white text-center' data-aos='zoom-in'>MarketPlace Management</h1>
            </div>

            <Category loadScreen={loadScreen} categoryDetails={category} pageSize={8} productDetails={marketplaceAllListings} pageName={name} />
        </>
    );
}

export default Category_mpm;