...
import { GetPurchase } from '../../handleSlice';
...
const Photo = ({ item }) => {
    ...
    useEffect(() => {
        const fetch = async () => {
            try {
                const response = await axios.get(`${url1}/viewmember_crm?mid=${item.seller_mid}`, {
                    headers: {
...
function Purchase() {
    ...
    useEffect(() => {
        const fetch = async () => {
            await dispatch(GetPurchase(mid))
        }
        fetch();
    }, [dispatch])
    const purchases = useSelector((state) => state.userDetails.GetPurchase);
    return (<>
                                ...
                                <div className='d-flex justify-content-between flex-wrap'>
                                    <div className='ms-md-3 '>
                                        ...
                                        <div>
                                            ...
                                            <div>Items Buyed : <span className='fw-bold'>{item.count}</span></div>
                                        </div>
                                        <p className='text-secondary mt-2 mb-0'>{item.date} / {item.time}</p>
                                    </div>
                                    <Photo item={item} />
                                </div>
                            </div>
